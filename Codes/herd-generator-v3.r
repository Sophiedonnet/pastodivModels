...
###################################################################
initfather = data.frame(cbind(unique(pop.table$father),0,0,"M"))
names(initfather) = names(pop.table)[c(1:3,5)]
initmother = data.frame(cbind(unique(pop.table$mother),0,0,"F"))
names(initmother) = names(pop.table)[c(1:3,5)]
herd = rbind(initfather,initmother,pop.table[,c(1:3,5)])
...
for (i in 1:n.gen){
  ...
  keptnewborn <- sample(nrow(newborn.table.ewe), n.newewe.kept)
  final.ewe.table <- newborn.table.ewe[keptnewborn, ]
  herdram <- newpop.table.ram[,c(1:3,5)]
  herdram[,2:3] <- 0
  herd <- rbind(herd,final.ewe.table[,c(1:3,5)],herdram)
...
names(herd)
herd$id
herd[herd$id==herd$mother[85],]


herd=herd[!duplicated(herd$id),]
ped = herd
ped$sexe
class(ped$id)
herd[85,]
LEV = unique(c((ped$id),(ped$father),(ped$mother)))
ped$id=as.numeric(factor(ped$id,levels=LEV))
ped$father=as.numeric(factor(ped$father,levels=LEV))
ped$mother=as.numeric(factor(ped$mother,levels=LEV))
names(ped) = c("ind","father","mother","sex")
ped[herd$father==0,2:3]=0
class(ped$sex)
